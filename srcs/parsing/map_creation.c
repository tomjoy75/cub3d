...
static int	_save_player_coord(t_datap *data, int i, int j, t_list *list)
{
	data->pos_x = i;
	data->pos_y = j;
...
static void	fill_the_map(int **map, t_list *list, t_datap *data)
{
	int	i;
	int	j;
	int	eol;
...
// {
// 	return (map[j][i] == EMPTY)
// }

static int	_wrong_elem_around(int **map, t_datap *data, int i, int j)
{
	if (i == 0 || j == 0 || i == data->map_len - 1
		|| j == data->map_height - 1)
	{
		if (_is_walkable(map[j][i]))
			return (1);
	}
	else
	{
		if (_is_walkable(map[j][i]) && (map[j - 1][i] == EMPTY ||
			map[j + 1][i] == EMPTY || map[j][i - 1] == EMPTY
			|| map[j][i + 1] == EMPTY))
			return (1);
		else if (map[j][i] == EMPTY && (\
			_is_walkable(map[j][i - 1]) || _is_walkable(map[j][i + 1]) || \
			_is_walkable(map[j - 1][i]) || _is_walkable(map[j + 1][i])))
			return (1);
	}
	return (0);
}
...
static void	check_map(int ***map, t_datap *data, t_list *list)
{
	int	i;
	int	j;
