...
/*   Updated: 2024/07/16 15:36:00 by jerperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	_wrong_elem_around(int **map_temp, t_data *data, int i, int j)
{
	...
	{
		if (is_walkable(map_temp[j][i]) && (map_temp[j - 1][i] == EMPTY ||
...
int	**create_map(t_data *data, t_list *list)
{
	...
	map_temp = allocate_map(data);
	if (NULL == map_temp)
		return (NULL);
	_fill_the_map(map_temp, list, data);
	if (_check_map(&map_temp, data))
		return (free_map(&map_temp, data->map->height), NULL);
	return (map_temp);
}
